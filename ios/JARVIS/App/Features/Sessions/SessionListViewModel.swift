...
public final class SessionListViewModel {
    ...
    public func bootstrap(config: APIConfig) async {
        ...
        self.client = c
        sessions = JarvisDAO.cachedSessions()
        await refresh()
    }
    ...
    public func delete(_ session: Session) async {
        ...
        do {
            ...
            sessions.removeAll { $0.session_id == session.session_id }
            JarvisDAO.delete(sessionID: session.session_id)
        } catch {
            errorMessage = error.localizedDescription
        }
    }
    ...
    public var grouped: [Group] {
        ...
        return JarvisListGroup.group(sessions, today: todayStart, yesterday: yesterdayStart)
    }
    ...
    static func messages(for sessionID: String) -> [ChatMessage] {
        ...
        case "demo-session-1":
            return [
                ...
                ChatMessage(
                    role: .assistant,
                    text: "JARVIS is your personal coding assistant. This demo account is running entirely from local sample data.",
                    terminal: .success,
                    isFinal: true
                )
            ]
