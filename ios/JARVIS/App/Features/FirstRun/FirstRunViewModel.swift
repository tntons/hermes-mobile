...
public final class FirstRunViewModel {
    ...
    public func saveAndContinue(appState: AppState) async {
        guard let url = URL(string: gatewayURLString.trimmingCharacters(in: .whitespacesAndNewlines)),
            ...
            errorMessage = "Enter a full URL starting with https:// (e.g. https://jarvis.example.com)."
            return
        }
        let token = bearerToken.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !token.isEmpty else {
            errorMessage = "Enter your JARVIS access token."
            return
        }
        ...
        errorMessage = nil
        appState.reloadAuth()
        JarvisLog.auth.info("first-run configured gateway=\(url.absoluteString, privacy: .public)")
    }
    ...
    public func continueAsMock(appState: AppState) {
        ...
        kc.isMockMode = true
        appState.reloadAuth()
        JarvisLog.auth.info("signed in with demo account")
    }
    #endif
