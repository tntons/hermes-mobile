...
///   .assistant — leading JARVIS mark + transparent text, animated reasoning
...
struct MessageCell: View {
    ...
    var body: some View {
        ...
        case .assistant:
            HStack(alignment: .top, spacing: 10) {
                avatar(symbol: "sparkles", tint: JarvisTheme.accent, fill: true)
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    if !message.text.isEmpty {
                        ...
                        if isStreaming {
                            JarvisActivityIndicator(label: "Generating response…", systemImage: "text.bubble")
                        }
    ...
    private var userBubble: some View {
        VStack(alignment: .trailing, spacing: 4) {
            Text(message.text)
                ...
                .background(JarvisTheme.userBubble, in: asymmetricBubble)
            HStack(spacing: 6) {
    ...
    private func avatar(symbol: String, tint: Color, fill: Bool) -> some View {
        ZStack {
            Circle()
                .fill(fill ? JarvisTheme.accentSoft.opacity(0.52) : JarvisTheme.surfaceElevated)
...
struct ToolCallCard: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // Header row — always visible. Mirrors the JARVIS Desktop's
            ...
            Button {
                ...
                HStack(spacing: 6) {
                    ...
                    Text(titleText)
                        .font(.caption.weight(.medium))
                        .foregroundStyle(isRunning ? JarvisTheme.accent : JarvisTheme.textSecondary)
                    Spacer(minLength: 4)
                    if let d = duration {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text(approval.command)
                ...
                .padding(8)
                .background(JarvisTheme.surfaceElevated, in: .rect(cornerRadius: 6))
        ...
        .padding(12)
        .background(JarvisTheme.surface, in: .rect(cornerRadius: 12))
        .overlay {
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            case .success:
                ...
                Text("Finished").font(.caption).foregroundStyle(JarvisTheme.textSecondary)
            case .cancelled:
                Image(systemName: "stop.circle").foregroundStyle(JarvisTheme.textSecondary)
                Text("Cancelled").font(.caption).foregroundStyle(JarvisTheme.textSecondary)
            case .error(let m):
