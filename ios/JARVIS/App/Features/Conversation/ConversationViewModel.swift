...
public final class ConversationViewModel {
    ...
    public func bootstrap(config: APIConfig) async {
        ...
        let c = JarvisClient(config: .init(gatewayURL: url, bearerToken: token))
        self.client = c

        // Show cached history immediately.
        let cached = JarvisDAO.cachedMessages(sessionID: sessionID)
        if !cached.isEmpty {
            messages = cached.map {
                ChatMessage(
    ...
    public func refreshHistory() async {
        ...
        do {
            ...
            messages = loaded
            JarvisDAO.upsertMessages(sessionID: sessionID, messages: detail.messages)
            ...
            JarvisLog.sse.debug("history refresh failed: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    private func send(text: String, appendingUserMessage: Bool) async {
        ...
        do {
            ...
            JarvisDAO.recordCursor(streamID: start.stream_id, sessionID: sessionID)
    ...
    private func sendMock(text: String, appendingUserMessage: Bool) async {
        ...
        messages[index].text = "Demo reply: I received \"\(text)\". Connect JARVIS later to talk to a real backend."
    ...
    private func handle(_ event: SSEEvent, envelope: SSEEventEnvelope, assistantID: UUID) {
        ...
        if let sid = currentStreamID, let eid = envelope.lastEventID {
            JarvisDAO.recordCursor(streamID: sid, sessionID: sessionID, lastEventID: eid)
        }
    ...
    private func finalizeStream(terminal: TerminalState, assistantID: UUID) {
        ...
        if let idx = messages.firstIndex(where: { $0.id == assistantID }) {
            ...
            messages[idx].isFinal = true
            JarvisDAO.appendMessage(messages[idx], sessionID: sessionID)
        }
        if let sid = currentStreamID {
            ...
            JarvisDAO.recordTerminal(streamID: sid, terminal: str)
            currentStreamID = nil
        }
        switch terminal {
        case .success:
