...
logger = logging.getLogger("hermes_bridge.webui_client")
...
class WebUIClient:
    ...
    async def _login(self) -> None:
        ...
        async with self._login_lock:
            ...
            if not password:
                raise RuntimeError(
                    "WEBUI_PASSWORD not set — cannot authenticate to hermes-webui"
                )
            ...
            while attempt < self._settings.webui_login_retries:
                ...
                try:
                    ...
                    last_err = exc
                    logger.warning(
                        "webui login attempt %d failed: %s", attempt, exc
                    )
                ...
                if resp.status_code == 200:
                    ...
                    logger.info("logged in to webui (cookie len=%d)", len(self._session_cookie_value or ""))
                    return
                else:
    ...
    async def stream(self, method: str, path: str, *, params: dict[str, Any] | None = None) -> httpx.Response:
