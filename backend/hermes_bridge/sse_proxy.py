...
logger = logging.getLogger("hermes_bridge.sse")
...
async def _stream_from_webui(
    ...
    deadline = settings.sse_proxy_write_deadline_seconds
    ...
    try:
        ...
        yield {"event": "apperror", "data": json.dumps({"type": "upstream_open_failed", "message": str(exc)})}
        return
                        ...
                        try:
                            ...
                            if apns and apns.enabled:
                                ...
                                if dt:
                                    ...
                                    body = payload.get("message") or "Hermes turn finished"
                                    asyncio.create_task(apns.send(dt, stream_id, str(title), str(body)))
                        except Exception:
                            logger.exception("APNs trigger failed")
                        try:
...
async def chat_stream(
    ...
    runs: RunsRegistry = Depends(get_registry),
) -> StreamingResponse:
